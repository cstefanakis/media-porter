...
class CountryServiceTest {
    ...
    @Test
    void autoCreateCountry_successfully(){
        //Arrest
        String czech = "Czech Republic";

        //Act
        Country autoCreatedCountry = countryService.autoCreateCountry(czech);

        //Assert
        assertNotNull(autoCreatedCountry.getId());
        assertEquals(czech, autoCreatedCountry.getEnglishName());
        assertNull(autoCreatedCountry.getNativeName());
        assertNull(autoCreatedCountry.getIso2Code());
        assertNull(autoCreatedCountry.getIso3Code());
    }

    @Test
    void autoCreateCountry_withExistEntity(){
        //Arrest
        String uk = "United Kingdom";

        //Act
        Country autoCreatedCountry = countryService.autoCreateCountry(uk);

        //Assert
        assertNotNull(autoCreatedCountry.getId());
        assertEquals("United Kingdom", autoCreatedCountry.getEnglishName());
        assertEquals("United Kingdom", autoCreatedCountry.getNativeName());
        assertEquals("GB", autoCreatedCountry.getIso2Code());
        assertEquals("GBR", autoCreatedCountry.getIso3Code());
    }

    @Test
    void autoCreateCountry_withNullProperty(){
        //Act
        Country autoCreatedCountry = countryService.autoCreateCountry(null);

        //Assert
        assertNull(autoCreatedCountry);
    }
